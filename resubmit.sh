...
do
echo '###############'
pre_name=`grep 'outputFile=' ./$1runJobs${num_job}.sh |awk '{print$20}'`
snd_name=${pre_name#*/} ## '#' remove right
file_prefix=${snd_name%.*}
echo $file_prefix
num_file=`ls /eos/user/c/chuw/ntuples/HHWWgg/${1}${file_prefix}_*.root |wc -l`
echo job:${num_job}
echo files:$num_file
pre_num=`grep -r nJobs ./$1runJobs${num_job}.sh |awk '{print$21}'`
real_num=${pre_num#*=}
echo $real_num
...
#  if [ $a ne 0 ]; then
file=/eos/user/c/chuw/ntuples/HHWWgg/$1${file_prefix}_$i.root ## '%' remove left
...
sed -i 's#longlunch#'"${a}"'#g' ${runjob_name%/*}_$num_job.sub
