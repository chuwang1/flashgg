...
snd_name=${pre_name#*outputFile=} ## '#' remove right
file_prefix=${snd_name%.root*}
echo $file_prefix
num_file=`ls ${file_prefix}_*.root |wc -l`
echo job:${num_job}
echo files:$num_file
pre_num=`grep -r nJobs ./$1runJobs${num_job}.sh`
snd_num=${pre_num#*nJobs=}
real_num=${snd_num%jobId=*}
echo $real_num
...
#  if [ $a ne 0 ]; then
file=${file_prefix}_$i.root ## '%' remove left
...
sed -i 's/.*JobFlavour.*/+JobFlavour = "'${a}'"/g' ${runjob_name%/*}_$num_job.sub
